...
module Examples.AG.PP.HaskellPP.AG.PrettyP
  ( -- * Pretty printing
    Pretty,
...
import Examples.AG.PP.Interface as P
...
instance Pretty HsExp where
        ...
        pretty (HsIf cond thenexp elsexp) =
           ((par >>|<< par >>|<< par)
            >>^<< 
             -- to reproduce bug: uncomment the two following lines, comment the third line. It should output the same, yet it hangs
             -- (((par >>|<< par >>^<< par >>-<< par)
             --             >>$<< [par, par >>-<< par])))  
